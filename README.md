...
- 使いたいハード環境の再現も可（Switchでファミコンのゲームができるみたいな＝ファミコンのような動作環境をソフトウェアで作ってSwichに入れてる）

  *ホスト（ハイパーバイザー）型*
  - CPU・メモリなどを独立したVMにする、**それぞれがOSをもつ**
  - アプリ同士の干渉がない
...
⇨どこでも同じ動作が保証される仕組み
    ...
    - Dockerfile：　イメージを作るための設計図。テキストファイルで環境を定義

    - **イメージ**：　コンテナを作るためのテンプレート。ソフトウェア、設定が入っている
...
2. Docker Hubから、Ubuntu公式イメージをダウンロード
   ...
   docker pull ubuntu
   ```
3. コンテナを起動ー　実行するコマンドを指定しないと停止するので、通常はrunのみで起動はしない
...
## バインドマウント
- ホストマシン（Dockerが動作しているPC）のファイル・ディレクトリを、コンテナ内にマウント（接続）する（≒共有）
